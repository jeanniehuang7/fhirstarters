...
//import org.hl7.fhir.r4.model.RespRate;
...
public class RespRateResourceProvider implements IResourceProvider {

   private Map<String, RespRate> myRespRates = new HashMap<String, RespRate>();
   private Firestore db;

   public RespRateResourceProvider(Firestore _db) {
      db = _db;
      ...
      myRespRates.put("1", theResp);
      myRespRates.put("2", myResp);

   }

   @Override
   public Class<? extends IBaseResource> getResourceType() {
      return RespRate.class;
   }
   ...
   @Read()
   public RespRate read(@IdParam IdType theId) {
      searchForRespRate(db, theId);

      RespRate retVal = myRespRates.get(theId.getIdPart());
